...
INSERT INTO openstreetmap_schema.tags (object_type, tag_key, multiplicity, tag_datatype_id, is_system)
-- object_type
SELECT ot.object_type, 'object_type', '1',    'CharacterString', true
FROM openstreetmap_schema.object_types ot
WHERE ot.object_type in ('AA_Objekt', 'AA_zeigtAufExternes', 'AA_modellart', 'AA_istAbgeleitetAus', 'AA_hatDirektUnten', 'AG_thema', 'AX_objekthoehe')

UNION ALL

-- type
SELECT ot.object_type, 'type','0..1', 'CharacterString', true
FROM openstreetmap_schema.object_types ot
WHERE ot.object_type in ('AA_Objekt', 'AA_zeigtAufExternes', 'AA_modellart', 'AA_istAbgeleitetAus', 'AA_hatDirektUnten', 'AG_thema', 'AX_objekthoehe')
...
-- admin_level
SELECT ot.object_type, 'admin_level','1', 'OSM_admin_level', true
  FROM openstreetmap_schema.object_types ot
...
-- boundary
SELECT ot.object_type, 'boundary','1', 'CharacterString', true
FROM openstreetmap_schema.object_types ot
WHERE ot.object_type in ('AX_KommunalesGebiet', 'AX_Gebiet_Bundesland', 'AX_Gebiet_Regierungsbezirk', 'AX_Gebiet_Kreis',
                         'AX_Gebiet', 'AX_Gebiet_Verwaltungsgemeinschaft', 'AX_KommunalesTeilgebiet', 'AX_Kondominium', 'AX_Gebietsgrenze')
